  ...
  return (
    <div className="group relative bg-[#1A2C38] border border-[#213743] hover:border-[#1475E1]/60 rounded-xl overflow-hidden shadow-lg transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-blue-500/10 flex flex-col">
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-[#0F212E]">
        ...
        <div className="absolute top-2 left-2 z-10 w-6 h-6 rounded-full bg-[#0F212E]/90 border border-cyan-500/40 p-0.5 shadow-md flex items-center justify-center backdrop-blur-sm">
          <ObsidianEmblemSvg className="w-full h-full" />
        </div>

        {/* Stake-Style Bottom-Left Playing Counter Bar */}
        <div className="absolute bottom-2 left-2 z-10 flex items-center gap-1 px-2 py-0.5 bg-black/70 backdrop-blur-md rounded-md border border-white/10 text-[9.5px] font-bold text-[#B1BAD3]">
        ...
        <button
          ...
          className="absolute top-2 right-2 p-1.5 rounded-lg bg-black/60 backdrop-blur-md text-slate-300 hover:text-amber-400 z-10 transition-colors"
        ...
        <Link
          href={`/game/${game.slug}`}
          className="absolute inset-0 z-20 flex items-center justify-center bg-black/50 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300"
        >
      ...
      <div className="p-2.5 bg-[#1A2C38] flex items-center justify-between">
        <div className="min-w-0 pr-1">
          <h3 className="text-xs font-bold text-white truncate group-hover:text-[#1475E1] transition-colors">
            {game.name}
          </h3>
          <p className="text-[10px] font-semibold text-[#557086] uppercase truncate">
            {game.provider_code}
          </p>
        </div>
        ...
        <Link
          href={`/game/${game.slug}`}
          className="p-1.5 bg-[#213743] hover:bg-[#1475E1] text-white rounded-lg transition-colors shrink-0"
        >
